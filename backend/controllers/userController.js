...
exports.registerUser = async (req, res) => {
  try {
    ...
    if (role === 'employer') {
      if (!companyEmail) {
        return res.status(400).json({ message: 'Company email is required for employers' });
      }
      ...
      if (!companyEmail.endsWith('.com')) {
        return res.status(400).json({ message: 'Please use your company email for employer registration' });
      }
    }
    ...
    if (existingUser) return res.status(400).json({ message: 'User or company email already exists' });
